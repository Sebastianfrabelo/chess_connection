...
int movCavalo(peca tab[][8], int new_x, int new_y, int turno, int pieceState[2][2]) {
    ...
    for (int i = 0; i < 4; i++) { // |x - new_x| + |y - new_y| = 3, 
        ...
        x = new_x + cav[i];
        y = new_y + 3 - abs(cav[i]); 
        if (x > 7 || x < 0) continue; //fora do campo
        if (y < 8 && y >= 0) {
            if (tab[x][y].jog == turno && tab[x][y].tipo == 'n') { //cavalo encontrado
    ...
    switch(numCav){ //quantos cavalos foram encontrados?
        case 0: 
            return 0;
            break;
        case 1:
...
int movRBQ(peca tab[][8], int new_x, int new_y, int turno, char tipo,int sq[], int size, int pieceState[2][2]) { 
    ...
    switch(numFound){ //quantas pecas foram encontradas?
	case 0:
	    return 0;
	    break;
	case 1:
...
int movPeao(peca tab[][8], int new_x, int new_y, int turno, int cap, int pieceState[2][2]) {
    ...
    switch(pawnNum){ //quantos peoes foram encontrados?
        case 0:
            return 0;
            break;
        case 1:
